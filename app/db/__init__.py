...
# Load .env if present (does nothing if the file doesn't exist, so this
# is safe in production where real env vars are injected by the host).
load_dotenv()
...
class DatabaseConfig:
    ...
    ssl_mode: str | None  # e.g. "require" for managed Postgres/MySQL
...
def load_db_config() -> DatabaseConfig:
    return DatabaseConfig(
        ...
        ssl_mode=_get("DB_SSL_MODE"),  # e.g. "require", "verify-full"
    )
...
# Toggle: allow write statements (INSERT/UPDATE/DELETE) to be generated
# and executed. Defaults to False — read-only NL2SQL is the safe default.
ALLOW_WRITE_QUERIES = _get_bool("ALLOW_WRITE_QUERIES", False)

# Max rows returned to the frontend, to avoid dumping huge result sets.
